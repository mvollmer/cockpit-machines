...
export function getTodayYearShifted(yearDifference: number) {
...
export function rephraseUI(key: string, original: string) {
    if (!(key in transform)) {
...
export function vmId(vmName: string) {
    return `vm-${vmName}`;
}

export function networkId(poolName: string, connectionName: string) {
    return `network-${poolName}-${connectionName}`;
}

export function storagePoolId(poolName: string, connectionName: string) {
    return `pool-${poolName}-${connectionName}`;
}

export function findMatchingNodeDevices(hostdev: VMHostDevice, nodeDevices: NodeDevice[]) {
...
function getVmDisksMap(vms: VM[], connectionName: ConnectionName) {
 ...
 */
export function getStorageVolumesUsage(vms: VM[], storagePool: StoragePool) {
    if (!storagePool)
        return { };
 ...
 */
export function getDiskFullName(disk: VMDisk): string {
    let name;
 ...
 * @returns {string}
 */
export function getDiskPrettyName(disk: VMDisk): string {
...
export function getVmStoragePools(connectionName: ConnectionName) {
    const { storagePools } = store.getState();
    return storagePools.filter(sp => sp && sp.name && sp.connectionName == connectionName && sp.active);
}

export function nicLookupByMAC(interfacesList: VMInterface[], mac: string) {
...
export function getStoragePoolPath(storagePools: StoragePool[], poolName: string, connectionName: ConnectionName) {
